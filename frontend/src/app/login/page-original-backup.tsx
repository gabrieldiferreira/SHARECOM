...
export default function LoginPage() {
  ...
  const syncAndRedirect = async (user: any) => {
    try {
      ...
      const res = await fetch('/api/user/sync', { method: 'POST', headers: { Authorization: `Bearer ${token}` }});
      if (res.ok) {
        const data = await res.json();
        if (data.user?.locale) document.cookie = `NEXT_LOCALE=${data.user.locale}; path=/; max-age=${60 * 60 * 24 * 365}`;
        if (data.user?.currency) localStorage.setItem('USER_CURRENCY', data.user.currency);
      }
    } catch (e) {
  ...
  const handleGoogleLogin = async () => {
    console.log('🔥 BUTTON CLICKED - handleGoogleLogin chamado');
    alert('Click registrado! Iniciando login...');
    ...
    try {
      ...
      if (isLocalhost || !isMobile) {
        ...
        if (result.user) syncAndRedirect(result.user);
  ...
  const handleBiometricLogin = () => {
    console.log('🔥 BUTTON CLICKED - handleBiometricLogin chamado');
    alert('Autenticação biométrica não suportada. Use login com Google.');
  };
              ...
              <div className="space-y-5">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="w-12 h-12 flex items-center justify-center rounded-xl transition-all active:scale-[0.98] hover:bg-white/5 bg-white/5 border border-white/10 text-white"
                    title="Biometria"
                  >
                    <Fingerprint size={18} />
                  </button>
