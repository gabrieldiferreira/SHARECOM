    ...
    try:
        ...
        for i, tx in enumerate(req.transactions):
            ...
            try:
                ...
                date_str = tx.transaction_date or ""

            values = [
                date_str,
                tx.merchant_name or "Desconhecido",
                tx.category or "Outros",
                "Entrada" if tx.transaction_type == "Inflow" else "Saída",
                tx.payment_method or "Desconhecido",
        ...
        for i, (cat, total) in enumerate(sorted(cat_totals.items(), key=lambda x: -x[1])):
            ws2.cell(row=11 + i, column=1, value=cat or "Outros")
...
def export_pdf(req: ExportRequest):
    try:
        ...
        safe_customer_name = (req.customer_name or "Cliente").strip()
        
        hash_doc = _generate_hash(
            safe_customer_name, data_geracao, len(req.transactions),
        )

        # Filtra e ordena transações
        valid_transactions = [t for t in req.transactions if t.transaction_date]
        sorted_tx = sorted(valid_transactions, key=lambda t: _parse_tx_date(t.transaction_date))
        ...
        def _header_footer(canvas, doc):
            ...
            canvas.drawRightString(
                ...
                f"Página {canvas.getPageNumber()} \u2022 Gerado em {data_geracao}",
            )
            canvas.restoreState()
        ...
        # ---- CABEÇALHO ----------------------------------------------------------
        elements.append(Paragraph(req.report_title.upper(), title_style))
        ...
        if cat_totals:
            ...
            for cat, total in sorted(cat_totals.items(), key=lambda x: -x[1]):
                ...
                cat_data.append([
                    Paragraph(cat or "Outros", cell_left),
        ...
        for tx in sorted_tx:
            ...
            m_name = tx.merchant_name or "Desconhecido"
            merchant = m_name[:30] if len(m_name) > 30 else m_name
            
            natureza = "Entrada" if tx.transaction_type == "Inflow" else "Saída"
            tx_data.append([
                Paragraph(date_str, cell_center),
                Paragraph(merchant, cell_left),
                Paragraph(tx.category or "Outros", cell_left),
